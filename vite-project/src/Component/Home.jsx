...
export default function Home() {
    ...
    const handleArray = (e) => {
        ...
        if (!data.text || !data.amount) return; // prevent empty push
                ...
                <section className="grid gap-6 md:grid-cols-3 items-stretch mb-8">
                    <div className="md:col-span-2 bg-white/80 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="mt-4 flex items-baseline gap-2">
                            <span className="text-3xl md:text-4xl font-semibold text-slate-800 dark:text-white">{`${totalIncome}`}</span>
                            <span className="ml-2 text-sm text-slate-500">as of now</span>
                        </div>

                        <div className="mt-6 grid grid-cols-2 gap-4">
                            <div className="p-4 bg-green-50/70 dark:bg-green-900/30 rounded-lg">
                                <p className="text-xs text-slate-600 dark:text-slate-300">Income</p>
                                <p className="mt-1 text-lg font-medium text-green-700 dark:text-green-300"> {income}</p>
                            </div>
                            <div className="p-4 bg-red-50/70 dark:bg-red-900/30 rounded-lg">
                                <p className="text-xs text-slate-600 dark:text-slate-300">Expenses</p>
                                <p className="mt-1 text-lg font-medium text-red-600 dark:text-red-300">{expenses}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white/80 dark:bg-slate-900/60 rounded-2xl p-6 shadow-lg border border-slate-100 dark:border-slate-800">
                        ...
                        <ul className="mt-4 space-y-3 max-h-52 overflow-y-auto">
                            <li className="flex items-center justify-between bg-slate-50/60 dark:bg-slate-800/40 p-3 rounded-lg border border-slate-100 dark:border-slate-700">
                                <div>
                                    <p className="font-medium text-slate-800 dark:text-slate-100">Sample Item</p>
                                    <p className="text-xs text-slate-500">#0001</p>
                                </div>
